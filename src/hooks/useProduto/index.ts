...
interface iUseProduto {
  ...
  GetProdutosSuperBusca: (filter?: iFilter<iProduto>) => Promise<iDataResultTable<iProduto>>;
  GetProdutos: (filter?: iFilter<iProduto>) => Promise<iDataResultTable<iProduto>>;
...
const GetProdutos = async (filter?: iFilter<iProduto>): Promise<iDataProduto> => {
  const FILTER = filter ? CreateFilter(filter) : '?$top=15&$inlinecount=allpages';

  const response = await api.get(`${ROUTE_GET_ALL_PRODUTO}${FILTER}`);

  const result: iDataProduto = {
...
const GetTabelasFromProduto = async (produto: iProduto): Promise<iTabelaVenda[]> => {
  let tabelas: iTabelaVenda[] = [];

  let sql: string = SQL_NORMAL;

  if (produto.FAB_BRUTO > 0 && produto.FABRICANTE?.NOME === 'MWM') sql = SQL_MWM;
...
const useProduto = create<iUseProduto>((set) => ({
  ...
  GetProdutosSuperBusca: (filter?: iFilter<iProduto>) => GetProdutosSuperBusca(filter),

...
export const ModalItemOrcamento: React.FC<iModalItemOrcamento> = ({ callback }) => {
    ...
    const [ItemValor, setItemValor] = useState<number>(0.0);
    ...
    useEffect(() => {
        ...
        if (!ObjectIsEmpty(CurrentItem.PRODUTO)) {
            ...
            setItemValor(
                (oldValue) => (oldValue = isNaN(CurrentItem.VALOR) ? 0 : CurrentItem.VALOR),
            );
        }
    }, []);
    ...
    const GetTabelas = () => {
        ...
        dispatch(
            SetCurrentItem({
                ...
                // SUBTOTAL: CurrentItem.VALOR * CurrentItem.QTD,
                // TOTAL: CurrentItem.VALOR * CurrentItem.QTD,
    ...
    const OnChangeValue = (e: React.FocusEvent<HTMLInputElement>) => {
        if (ItemValor < Number(TabelaSelected.value)) {
            toast.error('O valor não pode ser menor que o valor da tabela atual.', {
                position: 'bottom-right',
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
            ...
            dispatch(
                SetCurrentItem({
                    ...CurrentItem,
                    VALOR: ItemValor,
                    SUBTOTAL: ItemValor * CurrentItem.QTD,
                    TOTAL: ItemValor * CurrentItem.QTD,
                }),
            );
        }
    };
                        ...
                        >
                            ...
                            <FlexComponent width='20%' sm={{ width: '100%' }}>
                                <InputCustom
                                    ...
                                    type='number'
                                    onChange={(e) =>
                                        setItemValor(parseFloat(e.target.value.replace(',', '.')))
                                    }
